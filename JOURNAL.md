...
* Prototype déployé sur Streamlit Cloud
* API Gemini 2.5 Flash + clé masquée
* 4 testeurs v1 + itération v2 (contexte applicatif)

### Jour 3 (17/03) — ✅ FAIT

* 11 testeurs, post LinkedIn carrousel (2407 impressions)
* Export CSV Jira/Xray + import validé
* Prompt strict (plus de données inventées)

### Jour 4 (18/03) — ✅ FAIT

* Export Gherkin/BDD + 4 formats d'export
* 9 leads LinkedIn contactés en DM

### Jours 5-6 (19-20/03) — ✅ FAIT

* Newsletter #1 → 928 abonnés en 24h
* Mode guidé conceptualisé (retour Luis v4-v5)
* Performance optimisée (CSV/Gherkin à la demande)
* Design épuré (icônes SVG émeraude)

### Jours 7-8 (21-23/03) — ✅ FAIT

* Vidéo démo publiée sur LinkedIn
* MODE GUIDÉ LIVRÉ — dialogue interactif IA/testeur
* Post LinkedIn #2 vidéo publié
* Post #3 (hallucinations) + Newsletter #2 rédigés
* Smartesting/Lynqa a contacté — décliné
* Testeur 13 (Rezika) : "similaire à Copilot mais bien d'avoir un outil spécialisé"

### Jour 9 (23/03) — ✅ FAIT

* 6 nouveaux retours en une journée
* Mode guidé validé par Aissata et Jordan
* Aissata : cas d'usage réel — 600 tests manuels, 197 US à venir, sous-staffée
* Jordan : "bluffant", "nouveau mode très complet"
* Said Amouri : "très utile, outil d'accélération"
* Jean-Yves Garcin : "vraiment top", veut tester malgré filtre anti-IA
* Karima teste aujourd'hui depuis son PC pro (Bforbank)
* Romain : bug bouton démo identifié
* Lyne : retours UX — navigation, responsive mobile, cohérence langue FR/EN

### Jours 10-11 (24-25/03) — ✅ FAIT

* ~30 personnes contactées avec le lien de l'app
* Retours exploitables : Anass (valide "[À DÉFINIR PAR LE TESTEUR]"), Sim (mode collaboratif), Karima (cas passants OK, non-passants à creuser), Sidi (langue FR/EN, bouton micro), Rida (mode guidé + export validés), Najat (lien Gherkin/auto), Ngoné (mode guidé validé), Sara Bencheikh (compare avec Gemini/Copilot, préfère l'outil)
* Sara Bencheikh : "Vraiment rien à dire j'adore", compare favorablement avec Gemini et Copilot
* Ken Defossez : a présenté l'outil à son responsable client chez Amadeus, veut intégrer Jira/Octane → 1er signal d'adoption enterprise
* Karima : RDV visio proposé pour démo (créneaux vendredi 28/03 ou lundi 30/03)
* Anass : valide la logique "[À DÉFINIR PAR LE TESTEUR]", confirme le potentiel malgré le front prototype
* Bug langue confirmé par 2 testeurs (Lyne + Sidi) : US en anglais → tests en français
* Ajustement prompts app.py : section fonctionnelle ~70% / edge cases ~30%
* Réponse LinkedIn rédigée pour Julien Mer (structuration données en amont du LLM)

### Jour 12 (26/03) — ✅ FAIT

* **Newsletter** : 2 099 abonnés (+1 171 depuis J6)
* **Newsletter #2 publiée** : "Pourquoi l'IA invente dans tes cas de test (et comment l'arrêter)"
* **Post LinkedIn #3 publié** (hallucinations IA) — renvoie vers newsletter #2
* **Ken Defossez** : message détaillé reçu — confirme qu'il a présenté l'outil à son responsable client Amadeus. Veut intégrer Jira/Octane + conversion tests en Playwright/RobotFramework. Intéressé par la formation, va demander financement à son ESN. Call proposé (en attente créneaux).
* **Karima SORIANO** : visio calée lundi 30/03 à 17h30 (Google Meet). Message envoyé pour qu'elle prépare une US réelle + résultat outil avant le call.
* **Aissata SISSOKO PIOCT** : message envoyé — lien newsletter + proposition de point rapide avant son lot 3. Elle veut adapter l'outil à son contexte (comptabilité notariale, 197 US à venir). En attente de ses créneaux.
* **Marie-Eve Lafrance (Testeur 30)** : a testé l'outil. Retour positif. "Très rapide, très efficace." Observe que le résultat mode guidé ≈ mode direct → signal à creuser (a-t-elle rempli le contexte ?). Recommanderait à une petite entreprise sans contraintes d'outils. Question de suivi envoyée.
* **app.py mis à jour** : prompts bilingues (détection de langue automatique), numérotation des cas (CTF-001/CE-001/R-001), fix double-clic CSV/Gherkin, suppression auto de l'intro IA, gestion robuste JSON dans l'export CSV.
* **Veille concurrentielle** : Momentic (YC W24) — plateforme d'automatisation E2E en langage naturel, $15M Series A, 2 600 utilisateurs, clients Notion/Webflow/Retool. Pas concurrent direct (ils font l'exécution de tests, nous la génération de cas de test). Confirme que le marché QA + IA est chaud.
* **Décision** : Phase 1 objectifs atteints — bascule Phase 2 officielle après le call Karima (30/03).
...
* Le mélange FR/EN dans le Gherkin perturbe certains testeurs (retour Lyne, Sidi) — FIXÉ J12 (prompts bilingues)
...
* Newsletter à 2 099 abonnés en 12 jours — signal de demande très fort
* Marie-Eve signale que mode guidé ≈ mode direct → vérifier si les testeurs remplissent bien le contexte (possible problème d'UX)

### En attente

* Retour de Ken sur les créneaux pour un call (15-20 min)
* Call Karima — lundi 30/03 à 17h30 (CALÉ)
* Retour d'Aissata sur ses créneaux pour un point rapide
* Retour de Marie-Eve sur la question mode guidé vs direct (a-t-elle rempli le contexte ?)
* Retour de Luis sur le mode guidé
* Retour de Diawando avec VPN
* Retour d'Elodie — relancer début avril
* Relancer les "je vais tester" à 48h : Andriamaroson Fabius, Oumar Sidibe, Racchana, Bineta SALL
* Publier newsletter #3 (semaine prochaine)
...
* **Retour Jour 12 :** Message détaillé — travaille en régie DSI interne Amadeus, en charge des outils/process QA. Veut aussi convertir tests manuels → Playwright/RobotFramework. Formation : va demander financement ESN. Call proposé, en attente créneaux.
...
* **Action** : Navigation et responsive = limitations Streamlit. Cohérence langue → ✅ FIXÉ J12 (prompts bilingues).
...
* **Jour 12 :** Visio calée lundi 30/03 à 17h30. Message envoyé pour préparer une US réelle avant le call.
* **Action** : Lead enterprise Bforbank. Call lundi 30/03.
...
* **Jour 12 :** Veut un point rapide pour adapter l'outil à son contexte avant le lot 3. Intéressée par la formation (financement sur fonds propres, paiement en 4x). Abonnée newsletter. Message envoyé avec lien newsletter + proposition de créneau.
* **Action** : CAS D'USAGE CLÉ. En attente créneaux. Lead formation.
...
* **Date** : 25-26/03/2026
* **Retour J11 :** "Je vais aller regarder cela. Merci ! 😊"
* **Retour J12 :** A testé l'outil avec une user story. "J'ai bien aimé les deux options." "Le résultat était presque pareil" entre mode guidé et direct. "C'est un outil que je recommanderais à une petite entreprise." "Très rapide, très efficace."
* **Action** : Résultat guidé ≈ direct → question envoyée pour savoir si elle avait rempli le contexte. Signal de positionnement PME/freelance. Testé et validé.
...
| Testeurs confirmés | 10+ | 31 ✅ | J12 |
| Testeurs qui ont testé | 5+ | ~19 ✅ | J12 |
| Retours exploitables | 3+ | ~21 ✅ | J12 |
| Itérations produit | - | 11 | J12 |
| Formats d'export | - | 4 | J4 |
| Modes de génération | - | 2 (Guidé + Direct) | J8 |
| Posts LinkedIn | 3 | 3 ✅ | J12 |
| Newsletters publiées | 1 | 2 ✅ | J12 |
| Abonnés newsletter | - | 2 099 | J12 |
| Vidéo démo | - | ✅ | J8 |
| Mode guidé validé | - | ✅ (Aissata, Jordan, Sara, Rida, Ngoné) | J11 |
| Jours écoulés / 30 | - | 12/30 (40%) | - |
| Utilisateurs actifs | 100-500 | ~35 | J12 |
| Leads enterprise | - | 2 (Ken/Amadeus, Karima/Bforbank) | J12 |
| Leads formation | - | 2 (Ken, Aissata) | J12 |
| Taux conversion contact→test | - | ~25-30% | J12 |
| Calls discovery calés | - | 1 (Karima 30/03) | J12 |
...
16. **Prompts bilingues** : détection automatique de la langue de la US (Jour 12) — fixe bug FR/EN
17. **Bascule Phase 2** : objectifs Phase 1 atteints à J12 — bascule officielle après call Karima (30/03)

## Backlog

* ~~Prompt strict (Luis v2)~~ — FAIT
* ~~Export CSV Jira/Xray~~ — FAIT
* ~~Export Gherkin / BDD~~ — FAIT
* ~~Bouton démo pré-rempli~~ — FAIT
* ~~Optimisation performance~~ — FAIT
* ~~Vidéo démo~~ — FAIT
* ~~Mode guidé — dialogue interactif (Luis v4-v5)~~ — FAIT
* ~~Ajustement répartition prompts 70/30 (Jour 10)~~ — FAIT
* ~~Bug bouton démo (Romain)~~ — FIXÉ et validé (Jour 11)
* ~~Détection de langue (Lyne, Sidi)~~ — FIXÉ J12 (prompts bilingues)
* ~~Cohérence langue FR/EN dans le Gherkin (Lyne)~~ — FIXÉ J12 (prompts bilingues)
* ~~Masquer le prompt système visible (Anass)~~ — FIXÉ J11
* ~~Numérotation des cas de test (CTF-001, CE-001, R-001)~~ — FAIT J12
* ~~Fix double-clic CSV/Gherkin~~ — FAIT J12
...
* Internationalisation de l'interface (sidebar, header, boutons) — Phase 2
...
* **Ce qu'ils font** : Plateforme d'automatisation de tests E2E en langage naturel. Alternative à Selenium/Cypress/Playwright.
* **Levée** : $15M Series A (Standard Capital, Dropbox Ventures, Y Combinator)
* **Utilisateurs** : 2 600 — clients : Notion, Webflow, Retool, Quora, Xero
* **Positionnement** : Exécution de tests automatisés (aval du workflow QA)
* **Relation avec nous** : Pas concurrent direct. Nous = génération de cas de test (amont). Eux = exécution E2E (aval). Complémentaires.
* **Signal** : Valide que le marché QA + IA est chaud et financé.

---

## Phase 2 — Jours 31 à 60 : Validation marché (15/04 → 14/05/2026)

*(Bascule anticipée — objectifs Phase 1 atteints à J12. Lancement officiel Phase 2 après call Karima 30/03.)*

### Objectifs Phase 2
* Démos publiques LinkedIn (continu)
* 100 testeurs qui ont essayé le produit
* Observer : ce qui plaît, ce qui manque, ce qui casse
* Ajouter : génération de scripts automatisés (Playwright, Cypress, Selenium)
* Calls discovery : Karima (calé), Ken (en attente), Aissata (en attente)
